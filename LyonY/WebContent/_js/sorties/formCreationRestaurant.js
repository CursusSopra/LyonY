					...
					$('#idFormCreationRestaurant')
							.submit(
									function(e) {
										...
										if (!formOK) {
											$('#idFormAlertError').removeClass(
													'hidden');
											$('#idFormAlertError').fadeTo(
													10000, 500).slideUp(500);
											e.preventDefault();
										} else {
											$('#idFormAlertError').addClass(
													'hidden');
										}
									});

					$('#idFormCreationAmbiance')
							.submit(
									function(e) {
										e.preventDefault();
										...
										if (valLibAmbiance == '') {
											...
											$libAmbiance
													...
													.next('span')
													.html(
															'Vous devez renseigner un nouveau type !');
										} else {
											$libAmbiance.parent().parent()
													.parent().removeClass(
															'has-error');
											$libAmbiance.parent().next('span')
													.html('');
										}
										if (formOK) {
											$
