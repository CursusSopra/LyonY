...
public class Restaurant extends Sortie {
	...
	public void setListeDesPhotosDeRestaurant(
			List<Photo> listeDesPhotosDeRestaurant) {
	...
	public Restaurant(int id) throws SQLException {
		...
		String query = "SELECT l.idlieu, r.idsortie, r.idrestaurant, l.nom nomrestaurant,q.nom nomquartier, numero, "
				...
				+ "reservation, aemporter, AVG(av.note) AS notemoy,COUNT(av.note) AS nbavis	"
				+ "FROM restaurants r "
		...
		if (rs.next()) {
			...
			String query2 = "SELECT  idphoto " + "FROM photos p "
			...
			while (rs2.next()) {
				...
				listeDesPhotosDeRestaurant.add(new Photo(rs2
						.getInt("idphoto")));
	...
	public static List<Ambiance> getListeDesAmbiancesDeRestaurant()
			throws SQLException {
	...
	public static List<Restaurant> getListeDesRestaurants() throws SQLException {
		...
		while (rs.next()) {
			...
			while (rs2.next()) {
				...
				r.listeDesPhotosDeRestaurant.add(new Photo(rs2
						.getInt("idphoto")));
	...
	public void modifRestaurant(String nomrestaurant, int idAmbiance,
			int idQuartier, int numero, String voie, String codePostal,
			String ville, String description, String accessibilite,
			int prixmin, int prixmax, boolean reservation, boolean aemporter)
			throws SQLException {

		// Connexion à la BDD postgreSQL
		Connection cnx = PostgresConnection.getConnexion();

		String withQuery = "WITH A AS " + "(" + "select * FROM restaurants "
